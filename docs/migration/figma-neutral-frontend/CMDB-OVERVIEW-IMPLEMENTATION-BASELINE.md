...
| 2026-08-18 | `/workflow/instances` | 列表空态关闭共享 inbox，改用 Figma `git-branch` 实例 `6:26740` / glyph `6:26741` 的 20×20 原始 SVG，按 22px 放入空态图标槽；资产保存为 `frontend/public/figma-icons/workflow-git-branch.svg`，禁止手写 path。运行中/已完成 Tabs 按后续用户视觉反馈放在工具栏右侧，复用共享 `Tabs style="cmdb" size="sm"`。流程名称为空时依次回退到流程定义 Key 和「未命名流程」；较长业务标识保持单行省略，并通过原生 tooltip 提供完整内容。query key、权限与挂起/激活/终止合同不变；本轮不新增当前节点、发起人或持续时长字段。 | `workflow/instances/page.tsx`、`patterns.css`、`public/figma-icons/workflow-git-branch.svg`、`figma-neutral-m7-workflow-instances.test.cjs` | 用户已确认 |
