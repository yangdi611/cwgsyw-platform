...
| `BUG-FQA-079` | P1 | [`REM-P1-008`：CMDB 模型与动态属性合同收敛](./03-cmdb-assets/REM-P1-008-cmdb-model-attribute-contracts/README.md) | `VERIFIED` | FieldKey and enum option duplicate/concurrent boundaries plus used-option protection passed L1-L3; same-run CMDB-011 revalidation pending |
...
| `L4-OPS-016-001`：运维排班接受反向时间 | [`REM-P1-046`](./06-ops-collaboration/REM-P1-046-ops-roster-temporal-validation/README.md) | `VERIFIED` | create/update 统一拒绝 `endAt <= startAt`；合法跨日、完整 OPS-016、当前 backend 和精确清理 L1-L3 通过，待合并后同 run 重验。 |
| `L4-OPS-004-001`：256 字符运维任务标题落到数据库冲突并返回 HTTP 409 | [`REM-P1-047`](./06-ops-collaboration/REM-P1-047-ops-task-text-boundaries/README.md) | `VERIFIED` | 创建/更新统一 Unicode 255 字符合同；Java 21 L1、当前容器 API/UI 与产品清理 L1-L3 PASS，正文 `TEXT` 未新增未批准上限；待同 run `OPS-004` 重验。 |
| `L4-OPS-017-001`：组级用户可省略或伪造 `groupId` 导出非本组运维素材 | [`REM-P1-048`](./06-ops-collaboration/REM-P1-048-ops-material-group-scope/README.md) | `VERIFIED` | Controller 强制组级会话使用认证组，tenant/platform 保留显式或全租户范围；Java 21 L1、当前 backend、真实两组 API/UI/XLSX 与产品 API 精确清理 L1-L3 PASS，待同 run 重验。 |
| `L4-OPS-006-001`：组级普通读者可通过 direct id 读取跨组任务详情 | [`REM-P1-049`](./06-ops-collaboration/REM-P1-049-ops-task-detail-scope/README.md) | `VERIFIED` | 独立详情访问门禁与列表范围一致；跨组拒绝、public/related/read_group/read_all 正向、当前 backend、真实 API/UI 与精确清理 L1-L3 PASS，待同 run 重验。 |
| `L4-OPS-007-001`：任务 API 接受跨组负责人但真实 UI 候选缺失 | [`REM-P1-050`](./06-ops-collaboration/REM-P1-050-ops-cross-group-assignee-candidates/README.md) | `VERIFIED` | 组级任务创建者可读取同租户启用用户四字段最小候选并跨组指派；read-only 403、非法/停用/跨租户人员写前拒绝、当前 backend/frontend、真实 UI 与精确清理 L1-L3 PASS，待同 run重验。 |
| `L4-OPS-010-001`：真实双击确认产生两次状态、任务日志和审计副作用 | [`REM-P1-051`](./06-ops-collaboration/REM-P1-051-ops-confirm-idempotency/README.md) | `VERIFIED` | confirm 以租户约束事务行锁串行状态转换；Java 21 36/36、当前 backend、真实 API 并发/UI 双击、唯一日志/审计和精确清理 L1-L3 PASS，待同 run affected-only 重验。 |
| `L4-CMDB-040-001`：活动变更文档引用 CI 时实例 DELETE 仍返回 200 | [`REM-P1-052`](./03-cmdb-assets/REM-P1-052-cmdb-instance-reference-delete-guards/README.md) | `VERIFIED` | 实例行锁事务新增活动文档与日报引用保护，保留关系/设备和 HTTP 400 合同；Java 21 5/5、相关聚类 28/28、当前 backend、真实 API/UI、审计唯一性与精确清理 L1-L3 PASS，待同 run affected-only 重验。 |
| `L4-FLOW-002-001`：长审批意见导致日报审批 HTTP 500 | [`REM-P1-053`](./05-workflow-change/REM-P1-053-workflow-approval-audit-remark-boundary/README.md) | `VERIFIED` | 有界 Unicode 审计摘要修复通过 Java 21 8/8、Workflow/日报/Wiki/权限聚类 161/161、当前 backend 与真实日报/Wiki API/UI；待同 run affected-only 重验。 |
| `L4-CONFIG-004-001`：水印缺少角度与即时效果 | [`REM-P1-054`](./07-platform-integration/REM-P1-054-watermark-angle-preview-contract/README.md) | `VERIFIED` | angle API/UI/即时预览与既有 PDF 消费键修复通过 Java 14/14、frontend/backend build、当前容器和真实 Playwright；待同 run affected-only 重验。 |
| `L4-CMDB-008-002`：模型详情遗漏 `isDrawerShow`，真实实例抽屉无动态属性 | [`REM-P1-055`](./03-cmdb-assets/REM-P1-055-cmdb-model-detail-drawer-flag/README.md) | `VERIFIED` | 不改变历史 `BUG-FQA-*` 唯一主映射；DTO 映射、Java 33/33、当前 backend、真实十字段抽屉与精确清理 L1-L3 PASS，待同 run affected-only。 |
| `L4-CMDB-014-001`：`cmdb_attribute:*` canonical action 未被属性 API/UI 消费 | [`REM-P1-056`](./03-cmdb-assets/REM-P1-056-cmdb-attribute-canonical-action-guards/README.md) | `VERIFIED` | 不改变历史 `BUG-FQA-*` 唯一主映射；canonical 四动作、逐项缺权、legacy-only、六种真实 UI 显隐、双容器与精确清理 L1-L3 PASS；待同 run affected-only。 |
| `L4-WIKI-024-001`：无主组管理员创建子页时 `owner_group_id` 为空 | [`REM-P1-057`](./04-content-files/REM-P1-057-wiki-child-owner-group-inheritance/README.md) | `VERIFIED` | 不改变历史 `BUG-FQA-*` 唯一主映射；空组父组兜底、显式/setgid 回归、Java 96/96、当前 backend、真实 allow/ancestor-deny/non-leak 与精确清理 L1-L3 PASS；待同 run affected-only。 |
| `L4-RBAC-007-001`：同租户重复活动组名创建仍返回 200 | [`REM-P1-058`](./02-account-organization/REM-P1-058-group-active-name-uniqueness/README.md) | `VERIFIED` | 不改变历史 `BUG-FQA-*` 唯一主映射；trim-normalized create/update/restore/并发合同 L1 26/26、L2 114/114、当前 backend API/UI、日志与零残留 PASS；待同 run affected-only。 |
| `L4-XL-CMDB-003-001`：CI 与设备详情双向导航字段错误 | [`REM-P1-059`](./03-cmdb-device/REM-P1-059-cmdb-device-bidirectional-navigation/README.md) | `VERIFIED` | 不改变历史 `BUG-FQA-*` 唯一主映射；L1-L3、event/no-ff merge 和同 run affected-only 均通过。 |
| `L4-CHANGE-019-001`：并发提交和审批重复完成状态转换 | [`REM-P1-060`](./05-workflow-change/REM-P1-060-change-doc-submit-idempotency/README.md) | `VERIFIED` | submit/approve 使用 tenant-scoped 行锁复核状态；Java 21 L1 12/12、L2 52/52、当前 backend 完整终端链 1/1、归档与 manifest 0/0 通过，待同 run affected-only。 |
...
| `L4-XL-RBAC-002-001`：非主组有效成员无法使用 owner-group mode | [`REM-P1-069`](./01-security-authorization/REM-P1-069-multi-group-owner-mode-authorization/README.md) | `VERIFIED` | 不改变历史 `BUG-FQA-*` 唯一主映射；有效组 owner-mode、Java 21 120/120、生产 backend、真实双组撤销与清理 0/0 PASS。 |
| `L4-RBAC-025-XL-RBAC-006-001`：方法级功能权限拒绝缺少精确 reason code | [`REM-P1-070`](./01-security-authorization/REM-P1-070-method-security-functional-reason-code/README.md) | `VERIFIED` | 不改变历史 `BUG-FQA-*` 唯一主映射；filter/MVC 统一 `FUNCTION_PERMISSION_DENIED`，Java 21 定向簇、生产 backend、真实撤销最后 assignment 与精确清理 PASS；待同 run affected-only。 |
| `L4-CMDB-034-XL-CMDB-008-CLEANUP-001`：正式 Prometheus 同步告警缺少精确测试清理入口 | [`REM-P1-071`](./03-cmdb-assets/REM-P1-071-cmdb-alert-remediation-cleanup/README.md) | `VERIFIED` | 不改变历史 `BUG-FQA-*` 唯一主映射；platform/同租户/精确 runId 逻辑删除及审计已完成 Java 21、生产 backend、真实同步与零残留 L1-L3；待同 run affected-only。 |
| `L4-XL-EXPORT-004-001`：CSV 执行后失败结果不可下载 | [`REM-P1-072`](./03-cmdb-assets/REM-P1-072-cmdb-import-failed-row-download/README.md) | `VERIFIED` | 不改变历史 `BUG-FQA-*` 唯一主映射；batch tenant owner、失败结果有界 TTL、结构化 CSV 与公式注入防护已完成 Java 21 5/5、生产 backend、真实执行/下载与零残留 L1-L3；待同 run affected-only。 |
| `L4-CHANGE-020-001`：启用 `change_doc` binding 后提交未创建统一待办 | [`REM-P1-073`](./05-workflow-change/REM-P1-073-changedoc-unified-workflow/README.md) | `VERIFIED` | 原始 L4 `pending` 且 `matchingTask=null` FAIL 保留；统一流程启动、摘要/权限、旧旁路拒绝、完成回写、并发门禁与精确清理已通过 Java 21 L1 37/37、L2 124/124、共享回归 43/43、当前 backend 和真实 `/workflow/todo` 1/1；event `8c5aa594` no-ff 合并为 `1f4e3138`，同 run affected-only PASS。 |
| `L4-XL-CMDB-006-001`：CI 状态变更写 change 但未发送通知 | [`REM-P1-074`](./03-cmdb-assets/REM-P1-074-cmdb-status-change-notification-chain/README.md) | `VERIFIED` | 原始通知计数 0 保留；Java 21、当前 backend、真实状态/change/通知/UI、同状态静默与清理 0/0 L1-L3 PASS，待同 run affected-only。 |
| `L4-XL-EXPORT-002-001`：变更文档 PDF 缺表格且导出缺明确审批状态 | [`REM-P1-075`](./05-workflow-change/REM-P1-075-change-export-approval-table-contract/README.md) | `VERIFIED` | 不改变历史 BUG 主映射；DOCX/PDF 审批信息、PDF 表格和 application/plan 模板分区已通过 Java 21、当前 backend、真实统一审批/下载/归档、403/UI 隐藏与 manifest 0/0 L1-L3；待同 run affected-only。 |
| `L4-RBAC-013-001`：四个 canonical CMDB action 可分配但未被对应 API/UI 消费 | [`REM-P1-076`](./03-cmdb-assets/REM-P1-076-cmdb-canonical-permission-consumers/README.md) | `VERIFIED` | 不改变历史 BUG 主映射；import read/execute、impact、topology canonical allow 与 legacy substitute 403 已通过 Java 21、CMDB 聚类、当前 backend/frontend、真实 API/UI 和清理 0/0 L1-L3。RBAC-013 聚合仍需完整 99-action 当前 run 证据。 |
