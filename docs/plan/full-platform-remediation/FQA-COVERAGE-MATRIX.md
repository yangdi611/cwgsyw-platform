...
| `L4-CHANGE-020-001`：启用 `change_doc` binding 后提交未创建统一待办 | [`REM-P1-073`](./05-workflow-change/REM-P1-073-changedoc-unified-workflow/README.md) | `VERIFIED` | 原始 L4 `pending` 且 `matchingTask=null` FAIL 保留；统一流程启动、摘要/权限、旧旁路拒绝、完成回写、并发门禁与精确清理已通过 Java 21 L1 37/37、L2 124/124、共享回归 43/43、当前 backend 和真实 `/workflow/todo` 1/1；event `8c5aa594` 当前 no-ff 合并后同 run affected-only。 |
