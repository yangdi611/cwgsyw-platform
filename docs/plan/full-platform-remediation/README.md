...
当前规划含 49 个事件：4 个已关闭、45 个已验证。L4 发现已审批测试日报缺少完整 runId 而无法精确清理，已由 `REM-P1-031` 完成受限精确清理；集成后恢复最终 L4。先看 [缺陷覆盖矩阵](./FQA-COVERAGE-MATRIX.md) 确认历史 100 个缺陷与主事件的唯一映射；发布前全量复验仍是所有事件的最终共同门禁。
