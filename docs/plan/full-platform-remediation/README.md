...
当前规划含 63 个事件：4 个已关闭、59 个已验证。最终 L4 的 `CHANGE-006` 动态表格导出一致性缺口已由 `REM-P2-030` 完成 L1-L3：模板填充、程序化回退、实际 API DOCX 内容和真实浏览器下载均通过；待 no-ff 合并后重跑该用例。先看 [缺陷覆盖矩阵](./FQA-COVERAGE-MATRIX.md) 确认历史 100 个缺陷与主事件的唯一映射；发布前全量复验仍是所有事件的最终共同门禁。
