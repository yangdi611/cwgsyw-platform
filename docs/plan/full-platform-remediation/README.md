...
当前规划已覆盖源 run 的全部 100 个缺陷章节：45 个事件中，4 个已关闭、36 个已验证、0 个实施中、0 个验证中、5 个待实施、0 个阻塞。`REM-P2-009` 已完成 Wiki 搜索入口、URL/history 状态、授权分页、真实浏览器复验和 runId 清理。先看 [缺陷覆盖矩阵](./FQA-COVERAGE-MATRIX.md) 确认缺陷与主事件的唯一映射；发布前全量复验仍是所有事件的最终共同门禁。
