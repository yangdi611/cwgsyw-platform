...
当前规划含 56 个事件：4 个已关闭、52 个已验证。最终 L4 新发现 `REM-P2-023` 的日报导出缺少 UTF-8 下载头，已在独立分支完成 L1-L3，待 no-ff 合并后重跑受影响 L4；先看 [缺陷覆盖矩阵](./FQA-COVERAGE-MATRIX.md) 确认历史 100 个缺陷与主事件的唯一映射；发布前全量复验仍是所有事件的最终共同门禁。
