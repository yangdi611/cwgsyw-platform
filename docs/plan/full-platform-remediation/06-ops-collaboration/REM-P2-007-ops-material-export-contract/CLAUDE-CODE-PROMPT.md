# REM-P2-007 Claude Code 执行入口

在 `/Users/byron/AI/cwgsyw-platform` 只完成 `REM-P2-007：运维材料导出文件与摘要合同`。先读 `AGENTS.md`、`CLAUDE.md`、质量规则、总 `README/INDEX/FQA-COVERAGE-MATRIX`、本目录五份文档，以及 `defects.md` 中 `BUG-FQA-106` 和原始证据。

已批准范围：稳定 Content-Disposition/MIME/文件名；在工作簿写入范围、状态和汇总信息；验证空数据与有数据导出。非目标：不重做运维报表体系；不修改任务状态；不执行不可清理的全生命周期夹具。不得扩展到其他 REM。
