...
- Java 21 运维/通知模块扩展回归执行后，相关类均为 0 failure/0 error；唯一 error 是未改动 `OpsCalendarMaterialExportTest:45` 自 2026-07-16 存在的无效 Mockito stub（`UnnecessaryStubbingException`），该素材导出测试不在 `detail`/resolver 影响链，未作为本事件 PASS，也未在本事件顺手修改。
