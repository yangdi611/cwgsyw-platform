...
目标：账号与组织输入及显式清空合同，关闭 `BUG-FQA-015`、`BUG-FQA-044`、`BUG-FQA-085` 的共同根因。
...
当前：账号与组织 DTO 校验、数据库长度和显式清空语义不一致，空组名、非法邮箱、超长用户名或清空手机号会得到错误结果。

目标：用户输入被误报为 500，或接口返回成功但旧资料仍保留。对应的用户风险消失，API、服务、数据、权限、审计、前端入口和错误状态使用同一合同。

## 合同

- API / UI：相关入口对相同输入和权限返回一致、可解释的结果。
- 服务 / 数据：缺少 @Valid/@Email/@Size；MyBatis-Plus 默认 null 更新策略跳过显式清空字段。
...
候选文件 / 符号：`backend/src/main/java/com/cwgsyw/platform/module/user/**`、`backend/src/main/java/com/cwgsyw/platform/module/account/**`、`backend/src/main/java/com/cwgsyw/platform/module/org/**`、`frontend/src/app/(dashboard)/users/**`
...
| AC | 条件 |
|---|---|
| `AC-001` | BUG-FQA-015 的原始失败路径按目标合同通过。 |
| `AC-002` | 其余缺陷 BUG-FQA-044、BUG-FQA-085 分别有独立 PASS 证据。 |
| `AC-003` | 正常、边界、无权限/不存在及失败无副作用矩阵通过。 |
| `AC-004` | 空白/最大/超长用户名与组名；合法及非法 email；手机号设置后清空再回读；失败请求零持久化。 |
