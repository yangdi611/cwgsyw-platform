# REM-P1-006 Claude Code 执行入口

你现在位于 `/Users/byron/AI/cwgsyw-platform`，只持续完成 `REM-P1-006：权限消费、兼容别名与导航可达性收敛`。从 `IMPLEMENTATION-RECORD.md` 第一个未完成门禁继续；不要新建另一套 SPEC，不处理其他 REM 事件。
...
5. `defects.md` 中 `BUG-FQA-010`、`BUG-FQA-013`、`BUG-FQA-035`、`BUG-FQA-069`、`BUG-FQA-070`、`BUG-FQA-074` 及其原始证据。

## 已批准合同

目标：权限消费、兼容别名与导航可达性收敛。范围仅限：建立 canonical action 与兼容 alias 清单；逐项收敛 Controller guard、页面 query enabled 和导航可见性；对缺失 consumer 的权限做实现或下架决策。非目标：不重构统一授权引擎；不修改现有 assignment 数据；不顺带调整资源 ACL。
