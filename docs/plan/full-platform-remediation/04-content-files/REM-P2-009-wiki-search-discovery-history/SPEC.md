...
关闭 `BUG-FQA-071` 的共同根因，交付“Wiki 搜索入口与历史导航”。
...
当前：Wiki 首页没有搜索入口，搜索页又固定 router.replace，关键词无法通过浏览器历史恢复。

目标：用户难以发现搜索，后退/前进不能恢复查询上下文。对应的用户风险消失，API、服务、存储、UI、权限和审计保持一致。

## 合同、影响与实施

- 根因合同：首页信息架构和搜索页 URL 状态策略未形成产品合同。
- 候选文件 / 符号：`frontend/src/app/(dashboard)/wiki/page.tsx`、`frontend/src/app/(dashboard)/wiki/search/page.tsx`
- 范围：首页提供可发现搜索入口；关键词/分页写入 URL；明确 push/replace 策略并支持 back/forward；保持输入焦点与 debounce。
...
| AC | 条件 |
|---|---|
| `AC-001` | BUG-FQA-071 原始路径通过。 |
| `AC-002` | 全部子路径 分别有独立 PASS。 |
| `AC-003` | 正常、边界、无权/不存在、并发和失败零副作用通过。 |
| `AC-004` | 首页真实点击；输入/debounce/分页；back/forward/reload；空态与无权限。 |
