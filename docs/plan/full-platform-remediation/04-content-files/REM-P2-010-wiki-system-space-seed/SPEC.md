...
关闭 `BUG-FQA-078` 的共同根因，交付“Wiki 系统手册与只读空间种子”。
...
当前：前后端支持 system/readOnly/writeScope 分层，但运行数据没有系统手册空间。

目标：用户无法访问预期系统帮助，已实现只读分层无法验收。对应的用户风险消失，API、服务、存储、UI、权限和审计保持一致。

## 合同、影响与实施

- 根因合同：seed migration/初始化任务未创建或升级存量环境中的系统空间。
- 候选文件 / 符号：`backend/src/main/resources/db/migration/**`、`backend/src/main/java/com/cwgsyw/platform/module/wiki/**`、`frontend/src/app/(dashboard)/wiki/page.tsx`
- 范围：建立幂等系统空间 seed；支持存量环境补数与版本标记；验证只读保护和个人排序不受影响。
...
| AC | 条件 |
|---|---|
| `AC-001` | BUG-FQA-078 原始路径通过。 |
| `AC-002` | 全部子路径 分别有独立 PASS。 |
| `AC-003` | 正常、边界、无权/不存在、并发和失败零副作用通过。 |
| `AC-004` | 全新/升级/重复 seed；系统空间只读 API/UI；同名用户空间冲突；排序与权限。 |
