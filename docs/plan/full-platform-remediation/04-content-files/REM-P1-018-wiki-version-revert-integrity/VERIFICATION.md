...
| `AC-004` | 受影响模块 | L3 | 当前分支重建 backend，健康 `UP`；真实 API 验证详情、版本和导出；Playwright 经 nginx 登录、版本面板确认回退、刷新正文通过 | `PASS` |
| `AC-005` | impact/detect/cleanup/rollback | L3 | upstream impact 为 LOW；暂存后 `detect-changes` 标记 HIGH（9 条 Wiki 创建流程）且仅覆盖预期范围，已完成创建/保存/回退/导出/UI/API 回归；回滚为还原本事件提交 | `PASS` |
