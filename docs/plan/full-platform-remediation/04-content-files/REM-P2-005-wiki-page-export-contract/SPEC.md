...
关闭 `BUG-FQA-038` 的共同根因，交付“Wiki 当前页面导出合同”。
...
当前：页面“导出”操作运行时下载整个空间，而不是当前页面。

目标：按钮语义与下载内容不一致，用户可能误导出超出预期范围的信息。对应的用户风险消失，API、服务、存储、UI、权限和审计保持一致。

## 合同、影响与实施

- 根因合同：exportPage 客户端路由、构建产物或后端 Mapping 与页面按钮绑定不一致。
- 候选文件 / 符号：`frontend/src/lib/wiki-api.ts`、`frontend/src/app/(dashboard)/wiki/**`、`backend/src/main/java/com/cwgsyw/platform/module/wiki/WikiController.java`
- 范围：固定页面导出 endpoint 与参数类型；验证下载范围、文件名和 MIME；避免缓存/构建版本路由漂移。
...
| AC | 条件 |
|---|---|
| `AC-001` | BUG-FQA-038 原始路径通过。 |
| `AC-002` | 全部子路径 分别有独立 PASS。 |
| `AC-003` | 正常、边界、无权/不存在、并发和失败零副作用通过。 |
| `AC-004` | 页面/空间导出分别验证；中文标题文件名；无权限/不存在页面；正文与附件范围。 |
