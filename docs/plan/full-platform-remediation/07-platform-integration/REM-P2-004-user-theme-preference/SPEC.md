...
目标：用户主题切换与偏好持久化，关闭 `BUG-FQA-027` 的共同根因。
...
当前：应用没有可见主题开关或统一 ThemeProvider，已有深色适配能力无法通过产品入口使用。

目标：用户不能选择外观，报表、Markdown 和 BPMN 的深色可读性无法验收。对应的用户风险消失，API、服务、数据、权限、审计、前端入口和错误状态使用同一合同。

## 合同

- API / UI：相关入口对相同输入和权限返回一致、可解释的结果。
- 服务 / 数据：providers.tsx 未提供应用级主题状态，页头/个人菜单也没有切换控件。
...
候选文件 / 符号：`frontend/src/app/providers.tsx`、`frontend/src/components/layout/Header.tsx`、`frontend/src/components/**`
...
| AC | 条件 |
|---|---|
| `AC-001` | BUG-FQA-027 的原始失败路径按目标合同通过。 |
| `AC-002` | 其余缺陷 无 分别有独立 PASS 证据。 |
| `AC-003` | 正常、边界、无权限/不存在及失败无副作用矩阵通过。 |
| `AC-004` | light/dark/system 切换；刷新与重新登录持久化；报表/Wiki/BPMN 可读性；无 hydration 闪烁与 Console error。 |
