# REM-P2-006 Claude Code 执行入口

你现在位于 `/Users/byron/AI/cwgsyw-platform`，只持续完成 `REM-P2-006：通用配置拒绝的 HTTP 与业务码一致性`。从 `IMPLEMENTATION-RECORD.md` 第一个未完成门禁继续；不要新建另一套 SPEC，不处理其他 REM 事件。
...
5. `defects.md` 中 `BUG-FQA-022` 及其原始证据。

## 已批准合同

目标：通用配置拒绝的 HTTP 与业务码一致性。范围仅限：为白名单与参数拒绝返回稳定 4xx；统一相邻配置端点错误结构；保留成功响应合同。非目标：不扩大通用配置白名单；不修改配置值；不测试外部 SMTP/Prometheus。
