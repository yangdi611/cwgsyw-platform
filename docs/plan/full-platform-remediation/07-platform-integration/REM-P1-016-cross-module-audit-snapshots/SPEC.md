...
目标：跨模块写操作审计与快照完整性，关闭 `BUG-FQA-011`、`BUG-FQA-019`、`BUG-FQA-096` 的共同根因。
...
当前：IPAM、共享文件、运维等写路径缺失审计快照，CSV 导入还将非 JSON 文本写入 afterJson 并在业务写入后失败。

目标：管理员无法通过产品审计还原变更，批量导入可能实际成功却报告失败。对应的用户风险消失，API、服务、数据、权限、审计、前端入口和错误状态使用同一合同。

## 合同

- API / UI：相关入口对相同输入和权限返回一致、可解释的结果。
- 服务 / 数据：各模块自行拼装 AuditLog，before/after JSON、脱敏和事务顺序没有统一合同；审计 VO 也未完整暴露快照。
...
候选文件 / 符号：`backend/src/main/java/com/cwgsyw/platform/common/**`、`backend/src/main/java/com/cwgsyw/platform/module/audit/**`、`backend/src/main/java/com/cwgsyw/platform/module/ipam/**`、`backend/src/main/java/com/cwgsyw/platform/module/cmdb/service/CsvImportService.java`
...
| AC | 条件 |
|---|---|
| `AC-001` | BUG-FQA-011 的原始失败路径按目标合同通过。 |
| `AC-002` | 其余缺陷 BUG-FQA-019、BUG-FQA-096 分别有独立 PASS 证据。 |
| `AC-003` | 正常、边界、无权限/不存在及失败无副作用矩阵通过。 |
| `AC-004` | 各模块 CRUD 快照矩阵；敏感字段脱敏与大对象截断；CSV 单行/批次事务一致性；审计 API/UI 差异展示。 |
