# REM-P1-015 Claude Code 执行入口

你现在位于 `/Users/byron/AI/cwgsyw-platform`，只持续完成 `REM-P1-015：分页基础设施与审计筛选合同`。从 `IMPLEMENTATION-RECORD.md` 第一个未完成门禁继续；不要新建另一套 SPEC，不处理其他 REM 事件。
...
5. `defects.md` 中 `BUG-FQA-023`、`BUG-FQA-024`、`BUG-FQA-040`、`BUG-FQA-054` 及其原始证据。

## 已批准合同

目标：分页基础设施与审计筛选合同。范围仅限：补齐分页 interceptor 与 count 合同；实现审计 action/operator/keyword 过滤；验证其他 selectPage 消费者不回归。非目标：不补写业务审计快照；不改变审计保留周期；不修改通知权限。
