...
public class CiInstanceController {
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasPermission('cmdb_instance', 'read')")
    public R<CiInstanceDetailVO> getById(@PathVariable Long id,
                                         @AuthenticationPrincipal SecurityUser cu) {
    ...
    public R<CiInstanceDetailVO> create(@Valid @RequestBody CreateInstanceRequest req,
                                        @AuthenticationPrincipal SecurityUser cu) {
    ...
    public R<CiInstanceDetailVO> update(@PathVariable Long id,
                                        @RequestBody UpdateInstanceRequest req,
                                        @AuthenticationPrincipal SecurityUser cu) {
    ...
    @PreAuthorize("hasPermission('cmdb_instance', 'read')")
    public R<PageResult<ChangeHistoryVO>> history(
    ...
    public R<PageResult<ChangeHistoryVO>> globalChanges(
            ...
            @RequestParam(defaultValue = "20") int size,
            @AuthenticationPrincipal SecurityUser cu) {
        return R.ok(ciInstanceService.getGlobalChanges(model, operatorId,
                startDate, endDate, page, size, cu.getTenantId()));
    }
}
