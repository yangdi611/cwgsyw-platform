...
test('spatial versions route and history expose complete query and permission states', () => {
  ...
  assert.match(overlayCss, /\.cwgsyw-tooltip-positioner\s*\{[^}]*z-index:\s*80/)
  assert.match(overlaySource, /className="cwgsyw-tooltip-positioner"/)
  ...
  assert.match(overlaySource, /side=\{side \?\? \(followCursor \? 'bottom' : 'top'\)\}/)
