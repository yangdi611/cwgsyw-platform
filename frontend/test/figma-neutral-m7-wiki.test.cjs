...
test('wiki spaces leaves old visual entries and keeps APIs', () => {
  ...
  assert.match(page, /wiki-library\.svg/)
  assert.match(page, /授权 \$\{space\.name\}/)
  assert.match(page, /6:27530/)
  assert.ok(fs.existsSync(path.join(frontendRoot, 'public/figma-icons/wiki-library.svg')))
  assert.match(page, /queryKey: \['wiki-spaces'\]/)
  assert.match(page, /cwgsyw-wiki__header-actions/)
  assert.match(fs.readFileSync(path.join(frontendRoot, 'src/design-system/figma-neutral/components/patterns.css'), 'utf8'), /\.cwgsyw-wiki \.cwgsyw-page-header__row > \.cwgsyw-wiki__header-actions \{[\s\S]{0,80}margin-left: auto/)
