...
const pagePath = path.join(frontendRoot, 'src/app/(dashboard)/workflow/stats/page.tsx')
...
test('workflow stats leaves old visual entries', () => {
  ...
  assert.match(page, /queryKey: \['process-stats'\]/)
  assert.match(page, /\/workflow\/stats/)
  assert.match(page, /cwgsyw-workflow-stats__metrics/)
  assert.match(page, /cwgsyw-workflow-stats__panel/)
...
test('workflow stats renders Neutral shell', () => {
  const page = loadCompiled(pagePath)
  const html = renderToStaticMarkup(React.createElement(page.default))
  assert.match(html, /流程统计/)
