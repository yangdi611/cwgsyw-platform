...
test('analytics chrome uses Neutral trash close eye and chevron icons', () => {
  ...
  assert.match(dashboard, /leadingIcon="trash"/)
  ...
  assert.doesNotMatch(metrics, /Trash2/)
  assert.doesNotMatch(metrics, /<X className/)
  assert.match(metrics, /aria-label="编辑指标"/)
  assert.match(metrics, /aria-label="保存绑定"/)
  assert.match(metrics, /leadingIcon=\{<Save \/>\}/)
})
