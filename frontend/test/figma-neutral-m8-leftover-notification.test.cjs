...
test('notification bell uses Neutral icon-btn chrome instead of ad-hoc tailwind control', () => {
  ...
  assert.match(bell, /notification-bell__icon/)
  assert.match(bell, /cwgsyw-notification-dot/)
  assert.match(css, /\.cwgsyw-notification-dot/)
  assert.match(css, /var\(--cwgsyw-status-danger-fg\)/)
})
