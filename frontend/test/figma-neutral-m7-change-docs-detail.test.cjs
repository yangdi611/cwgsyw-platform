...
test('change-docs detail leaves old visual entries and keeps APIs', () => {
  ...
  assert.match(page, /cwgsyw-change-doc-detail__section/)
  ...
  const fields = fs.readFileSync(path.join(frontendRoot, 'src/components/change-doc/FieldList.tsx'), 'utf8')
  ...
  assert.match(bar, /size="sm"/)
  assert.match(picker, /size="sm"/)
  assert.match(fields, /size="sm"/)
  assert.match(css, /\.cwgsyw-change-doc-detail__head \{/)
  assert.match(css, /min-height: 34px/)
  const selector = fs.readFileSync(path.join(frontendRoot, 'src/components/cmdb/CiLinkSelector.tsx'), 'utf8')
  assert.match(selector, /cwgsyw-change-doc-ci-flow__list/)
  assert.match(selector, /layout="size"/)
  assert.doesNotMatch(selector, /cwgsyw-listbox--overlay/)
  assert.match(css, /\.cwgsyw-change-doc-ci-flow__list \{/)
