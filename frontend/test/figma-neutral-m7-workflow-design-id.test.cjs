...
const pagePath = path.join(frontendRoot, 'src/app/(dashboard)/workflow/design/[id]/page.tsx')
...
function loadCompiled(filePath) {
  ...
  Module._load = function load(request, parent, isMain) {
    ...
    if (request === 'next/dynamic') {
      return () => (props) => React.createElement('div', { 'data-testid': 'bpmn-editor' }, 'BPMN')
    }
    ...
    if (request === '@/lib/api') {
      return {
        get: async () => ({ data: { data: { records: [], id: '1', name: '变更审批流', key: 'changeDocApproval', category: '', description: '', version: 1, xml: '<xml/>' } } }),
        put: async () => ({ data: {} }),
      }
    }
...
test('workflow design id leaves old visual entries and keeps APIs', () => {
  ...
  assert.match(page, /\/workflow\/definitions\/key\/\$\{detail\.key\}\/update/)
  assert.match(page, /api\.get\('\/workflow\/definitions'/)
  assert.match(page, /initialXml=\{xml\}/)
  assert.match(page, /<bpmn:process id="/)
  assert.doesNotMatch(page, /@\/components\/design-system/)
  assert.doesNotMatch(page, /@\/components\/shared/)
  assert.doesNotMatch(page, /text-muted-foreground/)
  assert.doesNotMatch(page, /bg-muted/)
})

test('workflow design id renders Neutral loading shell', () => {
  const page = loadCompiled(pagePath)
  const params = Promise.resolve({ id: 'changeDocApproval' })
  params.status = 'fulfilled'
  params.value = { id: 'changeDocApproval' }
  const html = renderToStaticMarkup(React.createElement(page.default, { params }))
  assert.match(html, /加载流程定义/)
})
