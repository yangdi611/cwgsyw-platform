...
const pagePath = path.join(frontendRoot, 'src/app/(dashboard)/admin/ai/page.tsx')
...
function loadCompiled(filePath) {
  ...
  Module._load = function load(request, parent, isMain) {
    ...
    if (request === '@/lib/api') return { get: async () => ({ data: { data: [] } }), put: async () => ({}), post: async () => ({ data: { data: 'ok' } }) }
    if (request === '@tanstack/react-query') {
      return {
        useMutation: () => ({ mutate() {}, isPending: false }),
        useQuery: () => ({
          data: [{ provider: 'openai', providerLabel: 'OpenAI', baseUrl: 'https://api.openai.com/v1', model: 'gpt-4.1', enabled: true, systemPrompt: 'hi', configured: true }],
          isLoading: false,
          isError: false,
          refetch() {},
        }),
...
test('admin ai leaves old visual entries and keeps APIs', () => {
  const page = fs.readFileSync(pagePath, 'utf8')
  assert.match(page, /figma-neutral\/index\.css/)
  assert.match(page, /queryKey: \['ai-providers'\]/)
  assert.match(page, /\/admin\/ai\/providers/)
  assert.match(page, /hasPermission\('ai_config', 'read'\)/)
  assert.doesNotMatch(page, /@\/components\/design-system/)
  assert.doesNotMatch(page, /@\/components\/shared/)
  assert.doesNotMatch(page, /text-v2-/)
})

test('admin ai renders Neutral provider card', () => {
  const page = loadCompiled(pagePath)
  const html = renderToStaticMarkup(React.createElement(page.default))
  assert.match(html, /AI 网关配置/)
