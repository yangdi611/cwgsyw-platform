...
test('password page and form leave old visual entries and keep the password API contract', () => {
  ...
  assert.match(page, /cwgsyw-account-page--password/)
  ...
  assert.match(form, /cwgsyw-account-form/)
  assert.match(form, /size="sm"/)
  assert.equal(fs.existsSync(accountCssPath), true)
  assert.match(fs.readFileSync(accountCssPath, 'utf8'), /cwgsyw-account-page--password/)
  assert.match(fs.readFileSync(accountCssPath, 'utf8'), /--cwgsyw-account-content-width: 560px/)
...
test('password form renders Neutral fields and policy hints', () => {
  ...
  assert.match(html, /cwgsyw-account-form/)
