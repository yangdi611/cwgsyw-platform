...
test('one-off dialog renders Neutral fields without old overlay entries', () => {
  const dialog = loadCompiled(dialogPath)
  const html = renderToStaticMarkup(React.createElement(dialog.OneOffTaskDialog, { open: true, onOpenChange() {} }))
  assert.match(html, /快速创建一次性任务/)
  assert.match(html, /任务标题/)
  assert.match(html, /选择已发布模板/)
  assert.match(html, /选择执行人/)
  assert.match(html, /cwgsyw-dialog/)
  assert.doesNotMatch(html, /rounded-v2/)
})
