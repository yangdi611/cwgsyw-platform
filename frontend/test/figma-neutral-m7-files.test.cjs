...
test('files page leaves old visual entries and keeps file APIs', () => {
  ...
  assert.match(page, /files-folder\.svg/)
  assert.match(page, /6:26599/)
  assert.ok(fs.existsSync(path.join(frontendRoot, 'public/figma-icons/files-folder.svg')))
  ...
  assert.match(tree, /NeutralTooltip content=\{node\.name\}/)
  assert.match(page, /function TruncatedHint/)
  assert.match(page, /NeutralTooltip content=\{value\} className="cwgsyw-tooltip--pill" followCursor/)
  assert.match(page, /<TruncatedHint value=\{file\.name\}/)
  assert.match(page, /<TruncatedHint value=\{fileTypeLabel\(file\.fileType\)\}/)
  assert.match(page, /<TruncatedHint value=\{formatBytes\(file\.sizeBytes\)\}/)
  assert.match(page, /<TruncatedHint value=\{file\.createdByName \|\| '—'\}/)
  assert.match(page, /<TruncatedHint[\s\S]{0,80}value=\{new Date\(file\.createdAt\)/)
  assert.match(fs.readFileSync(path.join(frontendRoot, 'src/design-system/figma-neutral/components/patterns.css'), 'utf8'), /\.cwgsyw-files__table \.cwgsyw-td:not\(:last-child\) \.cwgsyw-tooltip-trigger \{[\s\S]{0,80}display: block/)
  assert.match(fs.readFileSync(path.join(frontendRoot, 'src/design-system/figma-neutral/components/patterns.css'), 'utf8'), /\.cwgsyw-files \.cwgsyw-files-tree__label \{[\s\S]{0,120}text-overflow: ellipsis/)
  assert.doesNotMatch(audit, /@\/components\/design-system/)
  assert.match(audit, /queryKey: \['shared-file-audit', page\]/)
  assert.match(audit, /AUDIT_PAGE_SIZE = 20/)
  assert.match(audit, /size: AUDIT_PAGE_SIZE/)
  assert.match(audit, /<Pagination/)
  ...
  assert.match(fs.readFileSync(path.join(frontendRoot, 'src/design-system/figma-neutral/components/patterns.css'), 'utf8'), /\.cwgsyw-files \.cwgsyw-pagination \{[\s\S]{0,80}padding: 12px 12px 16px/)
  assert.match(fs.readFileSync(path.join(frontendRoot, 'src/design-system/figma-neutral/components/patterns.css'), 'utf8'), /\.cwgsyw-files \.cwgsyw-page-header__row > \.cwgsyw-files__header-actions \{[\s\S]{0,80}margin-left: auto/)
  assert.doesNotMatch(preview, /> \$\{file\.createdByName\}/)
...
test('files page renders Neutral split workspace, folders and files', () => {
  ...
  assert.match(html, /cwgsyw-files__header-actions/)
