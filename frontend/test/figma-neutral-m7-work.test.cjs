...
test('work page and list leave old visual entries and keep work APIs', () => {
  ...
  assert.match(list, /style="cmdb"/)
  assert.match(list, /cwgsyw-work__tabs/)
  assert.match(list, /className="cwgsyw-work__filters"[\s\S]*className="cwgsyw-work__tabs"/)
  assert.match(list, /home-clipboard-check\.svg/)
  assert.match(list, /任务类型/)
  const patterns = fs.readFileSync(path.join(frontendRoot, 'src/design-system/figma-neutral/components/patterns.css'), 'utf8')
  const workFilters = patterns.slice(patterns.indexOf('.cwgsyw-work__filters {'), patterns.indexOf('.cwgsyw-work__filters >'))
  assert.match(workFilters, /overflow: visible/)
  assert.doesNotMatch(workFilters, /overflow-x: auto/)
  assert.match(patterns, /\.cwgsyw-work__tabs\s*\{[\s\S]*margin-left: auto/)
  assert.match(patterns, /table-layout: fixed/)
  assert.match(patterns, /\.cwgsyw-work__table\.cwgsyw-cmdb-table \.cwgsyw-table-wrap[\s\S]*overflow-y: hidden/)
  assert.match(drawer, /cwgsyw-work-drawer/)
})
