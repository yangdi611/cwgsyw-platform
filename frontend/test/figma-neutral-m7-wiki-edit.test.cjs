...
test('wiki editor leaves old visual entries and keeps APIs', () => {
  ...
  assert.match(page, /cwgsyw-wiki-edit__head/)
...
test('wiki editor renders Neutral chrome', () => {
  ...
  assert.match(html, /cwgsyw-wiki-edit__head/)
...
test('wiki editor toolbar maps heading commands to Figma icons', () => {
  const page = fs.readFileSync(pagePath, 'utf8')
  assert.match(page, /withWikiEditorToolbarIcon/)
  assert.match(page, /heading1: 'heading-1'/)
  assert.match(page, /heading6: 'heading-6'/)
  assert.match(page, /comment: 'message-square'/)
  assert.match(page, /cwgsyw-wiki-editor__toolbar-icon--\$\{name\}/)
  assert.doesNotMatch(page, /lucide-react/)
  assert.doesNotMatch(page, /Heading 1/)
})
