...
const pagePath = path.join(frontendRoot, 'src/app/(dashboard)/files/preview/[id]/page.tsx')
...
function loadCompiled(filePath) {
  ...
  Module._load = function load(request, parent, isMain) {
    ...
    if (request === 'next/navigation') return { useParams: () => ({ id: '12' }), useRouter: () => ({ push() {}, replace() {} }) }
    ...
    if (request === '@tanstack/react-query') {
      return {
        useQuery: ({ queryKey }) => {
          if (Array.isArray(queryKey) && queryKey[0] === 'file-detail') {
            return {
              data: {
                data: {
                  id: 12,
          ...
          if (Array.isArray(queryKey) && queryKey[0] === 'file-preview-content') {
            return { data: new Blob(['%PDF'], { type: 'application/pdf' }), isError: false, refetch() {} }
          }
    ...
    if (request === '@/lib/shared-file-content') {
      return { downloadSharedFile: async () => {}, fetchSharedFileBlob: async () => ({ blob: new Blob(['%PDF']) }) }
    }
...
test('file preview leaves old visual entries and keeps preview APIs', () => {
  const page = fs.readFileSync(pagePath, 'utf8')
  assert.match(page, /DetailDrawerPage/)
  assert.match(page, /figma-neutral\/index\.css/)
  assert.match(page, /queryKey: \['file-detail', id\]/)
  assert.match(page, /queryKey: \['file-preview-content', id\]/)
  assert.match(page, /fetchSharedFileBlob\(id, 'preview', 'preview'\)/)
...
test('file preview renders Neutral detail composition for a PDF', () => {
