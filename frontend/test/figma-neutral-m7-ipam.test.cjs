...
test('ipam page leaves old visual entries and keeps pool APIs', () => {
  ...
  assert.match(page, /ipam-network\.svg/)
  assert.match(page, /6:28340/)
  assert.ok(fs.existsSync(path.join(frontendRoot, 'public/figma-icons/ipam-network.svg')))
  ...
  assert.match(page, /figma-action-icon--eye/)
