...
test('task metrics leave old visual entries and keep APIs', () => {
  ...
  assert.match(manager, /TaskPanel/)
  assert.match(manager, /METRIC_STEPS/)
  assert.match(manager, /上一步/)
  assert.match(manager, /下一步/)
  assert.match(manager, /figma-icon--\$\{icon\}|icon: 'edit'/)
...
test('task metrics render Neutral workspace and existing metric', () => {
  ...
  assert.match(html, /来源字段/)
  assert.match(html, /指标目标/)
  assert.match(html, /期间预览/)
  ...
  assert.doesNotMatch(html, /按步骤完成/)
  assert.match(html, /已有指标/)
  const createAt = html.indexOf('cwgsyw-tasks-section__title">新建指标')
  const existingAt = html.indexOf('cwgsyw-tasks-section__title">已有指标')
  assert.notEqual(createAt, -1)
  assert.notEqual(existingAt, -1)
  assert.ok(createAt < existingAt)
})

test('task metrics keep the page title on the left when there is no header action', () => {
  const css = fs.readFileSync(path.join(frontendRoot, 'src/components/task-runtime/tasks.css'), 'utf8')
  assert.match(css, /page-header__row > :last-child:not\(h1\)/)
})
