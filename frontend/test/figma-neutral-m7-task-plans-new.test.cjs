...
test('new task plan page leaves old visual entries and keeps plan APIs', () => {
  ...
  assert.match(editor, /TaskPanel/)
  assert.match(editor, /cwgsyw-cmdb-wizard-steps/)
  assert.match(editor, /自定义周期/)
  assert.doesNotMatch(editor, /<Card/)
  assert.match(editor, /showSubtitle=\{false\}/)
  assert.doesNotMatch(editor, /高级 Cron/)
  ...
  assert.match(selector, /cwgsyw-tasks-cascader/)
  assert.match(selector, /先选择模型组/)
  assert.match(selector, /先选择模型/)
  assert.doesNotMatch(selector, /cwgsyw-stack-list__empty/)
  assert.doesNotMatch(selector, /<Card/)
