...
const pagePath = path.join(frontendRoot, 'src/app/(dashboard)/workflow/bindings/page.tsx')
...
test('workflow bindings leaves old visual entries', () => {
  const page = fs.readFileSync(pagePath, 'utf8')
  assert.match(page, /figma-neutral\/index\.css/)
  assert.match(page, /showBreadcrumb=\{false\}/)
  assert.match(page, /cwgsyw-workflow/)
  assert.match(page, /cwgsyw-workflow-bindings__table/)
  assert.match(page, /density="compact"/)
  assert.match(page, /workflow-pause\.svg|figma-action-icon--pause/)
  ...
  assert.match(page, /queryKey: \['workflow-bindings'\]/)
  assert.match(page, /\/workflow\/center\/bindings/)
...
test('workflow bindings renders Neutral shell', () => {
  const page = loadCompiled(pagePath)
  const html = renderToStaticMarkup(React.createElement(page.default))
  assert.match(html, /流程绑定/)
  assert.match(html, /变更文档审批/)
})
