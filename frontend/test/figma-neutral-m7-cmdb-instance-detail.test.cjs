...
test('cmdb instance detail leaves old visual entries', () => {
  ...
  assert.doesNotMatch(rackAssignment, /cwgsyw-stack-list/)
  assert.match(rackAssignment, /title="装入机柜"[\s\S]*description="选择目标机柜，并填写设备占用的 U 位。"[\s\S]*size="sm"/)
  assert.match(rackAssignment, /cwgsyw-cmdb-rack-dialog__u-grid/)
  assert.match(rackAssignment, /MotionConfig reducedMotion="user"/)
  assert.match(rackAssignment, /data-select-open=\{defSelectOpen \|\| rackSelectOpen \|\| undefined\}/)
  assert.match(rackAssignment, /open=\{defSelectOpen\}[\s\S]*onOpenChange=\{setDefSelectOpen\}/)
  assert.match(rackAssignment, /open=\{rackSelectOpen\}[\s\S]*onOpenChange=\{setRackSelectOpen\}/)
  assert.equal((rackAssignment.match(/cwgsyw-cmdb-dialog__flow-select/g) ?? []).length, 2)
  assert.doesNotMatch(rackAssignment, /<Select size="sm" overlay/)
  assert.match(rackAssignment, /variant="secondary"[\s\S]*取消/)
  assert.match(rackAssignment, /variant="primary"[\s\S]*确认装入/)
  ...
  assert.doesNotMatch(associations, /cwgsyw-stack-list/)
  assert.match(associations, /cwgsyw-cmdb-association-dialog/)
  assert.match(associations, /MotionConfig reducedMotion="user"/)
  assert.match(associations, /layout="size"/)
  assert.match(associations, /data-definition-open=\{definitionOpen \|\| undefined\}/)
  assert.match(associations, /open=\{definitionOpen\}/)
  assert.match(associations, /onOpenChange=\{setDefinitionOpen\}/)
  assert.doesNotMatch(associations, /<Select size="sm" overlay[\s\S]*placeholder="选择关联定义/)
  assert.match(associations, /cwgsyw-cmdb-association-dialog__candidates/)
  assert.match(associations, /aria-pressed=\{selectedPeerId === searchInst\.id\}/)
  assert.match(associations, /size="sm" variant="secondary"[\s\S]*取消/)
  assert.match(associations, /size="sm" variant="primary"[\s\S]*建立关联/)
  assert.match(associations, /cmdb-association-link-2\.svg/)
  assert.match(associations, /data-figma-node="6:27582"/)
  assert.match(associations, /width=\{22\}/)
  assert.match(associations, /height=\{12\}/)
  assert.match(associations, /<EmptyState title="暂无关联" showIcon=\{false\}/)
  ...
  assert.doesNotMatch(topology, /\bpreview\b|showNodeDetails/)
  assert.match(topology, /modelId: node\.modelId \?\? node\.model_id \?\? null/)
  assert.match(topology, /modelName: node\.modelName \?\? node\.model_name \?\? null/)
  assert.match(topology, /modelColor: node\.modelColor \?\? node\.model_color \?\? null/)
  assert.match(topology, /isRoot: node\.isRoot \?\? node\.is_root \?\? false/)
  assert.match(topology, /keyAttrs: node\.keyAttrs \?\? node\.key_attrs \?\? null/)
  ...
  assert.match(alerts, /cmdb-alert-circle\.svg/)
  assert.match(alerts, /data-figma-node="6:22984"/)
  assert.match(alerts, /width=\{22\}/)
  assert.match(alerts, /height=\{22\}/)
  assert.match(alerts, /showIcon=\{false\}[\s\S]*title="该实例暂无告警"/)
  assert.doesNotMatch(alerts, /<Card|<strong/)
  assert.match(resources, /cwgsyw-cmdb-instance-tab__resources/)
  assert.match(resources, /cmdb-resource-key\.svg/)
  assert.match(resources, /cmdb-resource-file-diff\.svg/)
  assert.match(resources, /data-figma-node=\{emptyIcon\.figmaNode\}/)
  assert.match(resources, /figmaNode: '6:27336'/)
  assert.match(resources, /figmaNode: '6:25779'/)
  assert.match(resources, /width: 22, height: 21/)
  assert.match(resources, /width: 18, height: 22/)
  assert.match(resources, /cwgsyw-cmdb-instance-tab__empty-icon-image/)
  assert.doesNotMatch(resources, /<svg|<path/)
  ...
  assert.match(patterns, /\.cwgsyw-cmdb-association-dialog__candidates[\s\S]*border: var\(--cwgsyw-border-width-default\) solid var\(--cwgsyw-border-subtle\)/)
  assert.match(patterns, /@keyframes cwgsyw-cmdb-dialog-flow-select-open[\s\S]*max-height: 0[\s\S]*translateY\(-4px\)/)
  assert.match(patterns, /\.cwgsyw-cmdb-dialog__flow-select \.cwgsyw-listbox[\s\S]*overflow-y: auto[\s\S]*animation: cwgsyw-cmdb-dialog-flow-select-open/)
  assert.match(patterns, /\.cwgsyw-cmdb-dialog__flow-select \.cwgsyw-listbox[\s\S]*overscroll-behavior: contain[\s\S]*scrollbar-gutter: stable[\s\S]*scrollbar-width: thin/)
  assert.match(patterns, /\.cwgsyw-cmdb-dialog__flow-select \.cwgsyw-listbox::\-webkit-scrollbar-thumb[\s\S]*background: var\(--cwgsyw-border-default\)/)
  assert.match(patterns, /\.cwgsyw-cmdb-association-dialog__candidates \.cwgsyw-btn\[aria-pressed="true"\][\s\S]*background: var\(--cwgsyw-bg-surface-hover\)/)
  assert.match(patterns, /\.cwgsyw-cmdb-rack-dialog__u-grid,[\s\S]*repeat\(2, minmax\(0, 1fr\)\)/)
  ...
  assert.match(patterns, /\.cwgsyw-cmdb-instance-tab__empty-icon[\s\S]*width: 24px[\s\S]*height: 24px/)
