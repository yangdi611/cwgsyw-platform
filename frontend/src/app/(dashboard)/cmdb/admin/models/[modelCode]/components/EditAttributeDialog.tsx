    ...
    <NeutralDialog
      ...
      footer={
        <>
          ...
          <Button type="button" disabled={!name.trim() || optionsMissing || isPending} onClick={handleUpdate}>
            保存
          </Button>
        </>
      }
      ...
      <div className="cwgsyw-form">
        <div className="cwgsyw-filter-grid">
          <Field label="字段标识" htmlFor="edit-key">
            <Input id="edit-key" value={attr.fieldKey} disabled />
          </Field>
          <Field label="类型" htmlFor="edit-type">
            <Input id="edit-type" value={FIELD_TYPES[attr.fieldType] ?? attr.fieldType} disabled />
          </Field>
          <Field label="显示名称" htmlFor="edit-name" required>
            <Input id="edit-name" placeholder="例如：CPU 核数" value={name} onChange={(event) => setName(event.target.value)} />
          </Field>
          <Field label="所属分组" htmlFor="edit-group">
            <Input id="edit-group" value={attr.groupName ?? attr.groupId} disabled />
          </Field>
        </div>
        {isEnum ? (
          <Field label="选项" htmlFor="edit-options" required>
            <Input id="edit-options" placeholder="生产,测试,开发" value={optionsText} onChange={(event) => setOptionsText(event.target.value)} />
          </Field>
        ) : null}
        <div className="cwgsyw-filter-grid">
          <Field label="默认值" htmlFor="edit-default">
            <Input id="edit-default" placeholder="可选" value={defaultValue} onChange={(event) => setDefaultValue(event.target.value)} />
          </Field>
          <Field label="排序" htmlFor="edit-sort">
            <Input id="edit-sort" type="number" min={0} value={sortOrder} onChange={(event) => setSortOrder(event.target.value)} />
          </Field>
        </div>
