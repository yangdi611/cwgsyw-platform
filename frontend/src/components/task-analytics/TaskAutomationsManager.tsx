...
function AutomationForm({ value, onChange, templates, users, onSubmit, busy, submitLabel }: {
  ...
  return <div className="grid gap-3 md:grid-cols-2">
    ...
    <label className="text-sm">触发器<Select value={value.triggerType} onChange={(triggerType) => onChange({ ...value, triggerType: triggerType as AutomationDraft['triggerType'], conditionField: '' })} options={['submission_approved', 'task_completed', 'metric_threshold']} /></label>
    ...
    <label className="text-sm">动作<Select value={value.actionType} onChange={(actionType) => onChange({ ...value, actionType: actionType as AutomationDraft['actionType'] })} options={['create_task', 'notify']} /></label>
    ...
    {value.conditionField && <><label className="text-sm">条件运算符<Select value={value.conditionOperator} onChange={(conditionOperator) => onChange({ ...value, conditionOperator })} options={['eq', 'ne', 'gt', 'gte', 'lt', 'lte']} /></label><label className="text-sm">条件值<Input value={value.conditionValue} onChange={(event) => onChange({ ...value, conditionValue: event.target.value })} /></label></>}
...
function Select({ value, onChange, options }: { value: string; onChange: (value: string) => void; options: string[] }) { return <select className={selectClass} value={value} onChange={(event) => onChange(event.target.value)}>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select> }
