...
export function TaskPlanEditor({ planId }: TaskPlanEditorProps) {
  ...
  if (planId && detail.isError) {
    return <ErrorState title="计划加载失败" description="无法读取任务计划。" retry={<Button type="button" variant="secondary" onClick={() => void detail.refetch()}>重试</Button>} />
  }
  if (templates.isError) {
    return <ErrorState title="模板选项加载失败" description="无法读取已发布模板。" retry={<Button type="button" variant="secondary" onClick={() => void templates.refetch()}>重试</Button>} />
  }
  if (approvalSchemes.isError) {
    return <ErrorState title="审批方案加载失败" description="无法读取审批方案。" retry={<Button type="button" variant="secondary" onClick={() => void approvalSchemes.refetch()}>重试</Button>} />
  }
...
function TaskPlanEditorForm({
  ...
  const approvalOptions = [
    ...
    ...approvalSchemes.map((scheme) => ({
      ...
      label: `${scheme.name} · v${scheme.latestVersion?.version ?? '-'}`,
    })),
  ]
    ...
    <FormSettingsPage
          ...
          breadcrumb={
            <Breadcrumb
              items={[
                ...
                { href: '/tasks/plans', label: '任务计划' },
                { label: planId ? initial.name : '新建计划' },
              ]}
            />
          }
      ...
      form={
        <div className="cwgsyw-split cwgsyw-split--nav">
          <nav className="cwgsyw-split__pane" aria-label="计划配置步骤">
            <div className="cwgsyw-stack-list">
              {STEPS.map((label, index) => (
                ...
                >
                  <span className="cwgsyw-type-label-xs">{index < step ? '完成' : index + 1}</span>
                  <strong>{label}</strong>
                </Button>
              ))}
            </div>
          ...
          <div className="cwgsyw-form">
            {step === 0 ? (
              <Card title="基础信息" description="选择已发布模板和可选审批方案；计划激活后始终固化对应版本。">
                <div className="cwgsyw-form">
                  <div className="cwgsyw-permission-grid">
                    <Field label="计划名称" required state={editable ? 'default' : 'disabled'}>
                      <Input disabled={!editable} value={payload.name} placeholder="例如：数据库每日巡检" onChange={(event) => setPayload({ ...payload, name: event.target.value })} />
                    </Field>
                  ...
                  <Field label="计划说明" state={editable ? 'default' : 'disabled'}>
                    <Textarea disabled={!editable} rows={4} value={payload.description || ''} placeholder="说明任务目的、适用范围和执行要求" onChange={(event) => setPayload({ ...payload, description: event.target.value })} />
                  </Field>
                </div>
              </Card>
            ) : null}
            ...
            {step === 1 ? (
              <Card title="周期与截止时间" description="支持一次性、常用周期、高级 Cron 和节假日前后规则。">
                <div className="cwgsyw-form">
                  <div className="cwgsyw-permission-grid">
                    <Field label="周期类型" state={editable ? 'default' : 'disabled'}>
                      <Select disabled={!editable} value={payload.scheduleType} options={SCHEDULE_OPTIONS} onChange={(value) => selectScheduleType(value as ScheduleType)} />
                    </Field>
                    <Field label="生效日期" state={editable ? 'default' : 'disabled'}>
                      <Input disabled={!editable} type="date" value={payload.startDate || ''} onChange={(event) => setPayload({ ...payload, startDate: event.target.value })} />
                    </Field>
                    <Field label="结束日期" state={editable ? 'default' : 'disabled'}>
                      <Input disabled={!editable} type="date" value={payload.endDate || ''} onChange={(event) => setPayload({ ...payload, endDate: event.target.value || undefined })} />
                    </Field>
                  </div>
                  ...
                  <div className="cwgsyw-permission-grid">
                    <Field label="截止偏移（小时）" state={editable ? 'default' : 'disabled'}>
                      <Input disabled={!editable} type="number" min={0} max={8760} value={numberValue(schedule.dueAfterHours, 24)} onChange={(event) => setSchedule('dueAfterHours', Number(event.target.value))} />
                    </Field>
                    <Field label="提前生成（天）" state={editable ? 'default' : 'disabled'}>
                      <Input disabled={!editable} type="number" min={0} max={365} value={payload.generateAheadDays ?? 7} onChange={(event) => setPayload({ ...payload, generateAheadDays: Number(event.target.value) })} />
                    </Field>
            ...
            {step === 2 ? (
              <>
                <Card title="执行对象" description="决定每个 occurrence 生成每人一份、每组一份、共享一份或单人一份任务。">
                  <div className="cwgsyw-form">
                    <div className="cwgsyw-permission-grid">
                ...
                <Card title="CI 范围（可选）" description="模型组、模型和单个 CI 可混合多选；上级范围自动覆盖下级并去重。">
                  {editable ? (
                    ...
                    <p className="cwgsyw-type-body-sm">已配置 {ciSelections.length} 个范围选择。</p>
                  )}
                </Card>
              </>
            ) : null}
            ...
            {step === 3 ? (
              <>
                <Card title="提醒与升级" description="提醒配置随计划保存，后续由统一通知 outbox 幂等投递。">
                  <div className="cwgsyw-permission-grid">
                    <Field label="到期前提醒（小时）" state={editable ? 'default' : 'disabled'}>
                      <Input disabled={!editable} type="number" min={0} value={numberList(payload.reminderConfig?.beforeDueHours)[0] ?? 2} onChange={(event) => setPayload({ ...payload, reminderConfig: { ...payload.reminderConfig, beforeDueHours: [Number(event.target.value)] } })} />
                    </Field>
                    <Field label="逾期升级（小时）" state={editable ? 'default' : 'disabled'}>
                      <Input disabled={!editable} type="number" min={0} value={numberValue(payload.escalationConfig?.afterOverdueHours, 24)} onChange={(event) => setPayload({ ...payload, escalationConfig: { ...payload.escalationConfig, afterOverdueHours: Number(event.target.value) } })} />
                    </Field>
                    <Switch
                      disabled={!editable}
                ...
                <Card
                  title="生成预览"
                  description={`${scheduleDescription} · 展示 occurrence、执行对象、截止时间与 CI 命中。`}
                  headerAction={
                ...
                >
                  {preview ? <PreviewPanel preview={preview} /> : <p className="cwgsyw-type-body-sm">点击“刷新预览”验证计划配置。</p>}
                </Card>
              </>
            ) : null}

            <div className="cwgsyw-form__actions">
              <Button type="button" variant="ghost" disabled={step === 0} onClick={() => setStep((value) => value - 1)}>上一步</Button>
              {step < STEPS.length - 1 ? (
                <Button type="button" variant="primary" disabled={!canNext} onClick={() => setStep((value) => value + 1)}>下一步</Button>
              ) : editable ? (
                <Button type="button" variant="primary" disabled={save.isPending || !basicValid || !assignmentValid} onClick={() => save.mutate()}>
  ...
  if (type === 'once') {
    return (
      <Field label="执行时间" state={disabled ? 'disabled' : 'default'}>
        <Input disabled={disabled} type="datetime-local" value={textValue(config.datetime)} onChange={(event) => onChange('datetime', event.target.value)} />
      </Field>
    )
  }
  if (type === 'cron') {
    return (
      <Field label="Spring 六段 Cron" state={disabled ? 'disabled' : 'default'}>
        <Input disabled={disabled} value={textValue(config.expression)} placeholder="0 0 9 * * MON-FRI" onChange={(event) => onChange('expression', event.target.value)} />
      </Field>
    )
  }
    ...
    return (
      <div className="cwgsyw-permission-grid">
        <Field label="相对方向" state={disabled ? 'disabled' : 'default'}>
          <Select disabled={disabled} value={textValue(config.relative, 'before')} options={Object.entries(RELATIVE_LABELS).map(([value, label]) => ({ value, label }))} onChange={(value) => onChange('relative', value)} />
        </Field>
        <Field label="第 N 个工作日" state={disabled ? 'disabled' : 'default'}>
          <Input disabled={disabled} type="number" min={1} value={numberValue(config.offsetWorkdays, 1)} onChange={(event) => onChange('offsetWorkdays', Number(event.target.value))} />
        </Field>
    ...
    <div className="cwgsyw-permission-grid">
      ...
      {type === 'weekly' ? (
        <Field label="星期" state={disabled ? 'disabled' : 'default'}>
          <Select disabled={disabled} value={textValue(config.weekday, 'MON')} options={Object.entries(WEEKDAY_LABELS).map(([value, label]) => ({ value, label }))} onChange={(value) => onChange('weekday', value)} />
        </Field>
      ) : null}
      ...
      {type === 'monthly' && config.position === 'day_of_month' ? (
        <Field label="每月第几日" state={disabled ? 'disabled' : 'default'}>
          <Input disabled={disabled} type="number" min={1} max={31} value={numberValue(config.day, 1)} onChange={(event) => onChange('day', Number(event.target.value))} />
        </Field>
      ) : null}
      {type === 'yearly' ? (
        <>
          <Field label="月份" state={disabled ? 'disabled' : 'default'}>
            <Input disabled={disabled} type="number" min={1} max={12} value={numberValue(config.month, 1)} onChange={(event) => onChange('month', Number(event.target.value))} />
          </Field>
...
function TimeField({ config, disabled, onChange }: { config: Record<string, unknown>; disabled: boolean; onChange: (key: string, value: unknown) => void }) {
  return (
    <Field label="执行时间" state={disabled ? 'disabled' : 'default'}>
      <Input disabled={disabled} type="time" value={textValue(config.time, '09:00')} onChange={(event) => onChange('time', event.target.value)} />
    </Field>
  )
}
...
function OptionGrid({ title, options, selected, disabled, onToggle }: { title: string; options: Array<{ id: number; label: string; meta?: string }>; selected: number[]; disabled: boolean; onToggle: (id: number) => void }) {
  return (
    <div className="cwgsyw-form">
      <p className="cwgsyw-type-label-sm">{title}</p>
      <div className="cwgsyw-stack-list">
        {options.map((option) => (
          <Checkbox
            key={option.id}
...
function PreviewPanel({ preview }: { preview: TaskPlanPreview }) {
  return (
    <div className="cwgsyw-form">
      <Alert tone="info" title={`模板“${preview.templateName}”将生成任务`} description={`以下 5 个 occurrence 共生成 ${preview.totalTaskCount} 份任务。`} showDismiss={false} />
      {preview.warnings.map((warning) => (
        <Alert key={warning} tone="warning" title="预览警告" description={warning} showDismiss={false} />
      ))}
      <div className="cwgsyw-stack-list">
        {preview.occurrences.map((occurrence) => (
          <article key={occurrence.occurrenceAt} className="cwgsyw-card cwgsyw-card--sm">
            <div className="cwgsyw-inline-controls">
              <strong>{new Date(occurrence.occurrenceAt).toLocaleString('zh-CN')}</strong>
              <span className="cwgsyw-type-label-xs">截止：{new Date(occurrence.dueAt).toLocaleString('zh-CN')}</span>
              <span className="cwgsyw-type-label-xs">{occurrence.taskCount} 份任务 · {occurrence.ciCount} 个 CI</span>
            </div>
            <div className="cwgsyw-inline-controls">
