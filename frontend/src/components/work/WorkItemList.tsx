...
import { Input, Select, SelectContent, SelectItem, SelectTrigger, SelectValue, StatusBadge } from '@/components/design-system'
...
export function WorkItemList() {
  ...
  return <div className="space-y-6">
    ...
    <FilterBar>
      ...
      <Select value={status} onValueChange={(value) => { setStatus(value ?? 'all'); resetPage() }}><SelectTrigger className="w-36"><SelectValue>{(value: string) => value === 'all' ? '全部状态' : statusLabel(value)}</SelectValue></SelectTrigger><SelectContent><SelectItem value="all">全部状态</SelectItem><SelectItem value="not_started">未开始</SelectItem><SelectItem value="in_progress">进行中</SelectItem><SelectItem value="changes_requested">待修改</SelectItem><SelectItem value="in_review">审批中</SelectItem><SelectItem value="completed">已完成</SelectItem><SelectItem value="cancelled">已取消</SelectItem></SelectContent></Select>
      <Select value={priority} onValueChange={(value) => { setPriority(value ?? 'all'); resetPage() }}><SelectTrigger className="w-32"><SelectValue>{(value: string) => value === 'all' ? '全部优先级' : priorityLabel(value)}</SelectValue></SelectTrigger><SelectContent><SelectItem value="all">全部优先级</SelectItem><SelectItem value="low">低</SelectItem><SelectItem value="normal">普通</SelectItem><SelectItem value="high">高</SelectItem><SelectItem value="critical">紧急</SelectItem></SelectContent></Select>
      <Select value={overdue} onValueChange={(value) => { setOverdue(value ?? 'all'); resetPage() }}><SelectTrigger className="w-28"><SelectValue>{(value: string) => ({ all: '全部时效', yes: '已逾期', no: '未逾期' })[value] ?? value}</SelectValue></SelectTrigger><SelectContent><SelectItem value="all">全部时效</SelectItem><SelectItem value="yes">已逾期</SelectItem><SelectItem value="no">未逾期</SelectItem></SelectContent></Select>
      <Select value={templateId} onValueChange={(value) => { setTemplateId(value ?? 'all'); resetPage() }}><SelectTrigger className="w-44"><SelectValue placeholder="全部模板">{(value: string) => value === 'all' ? '全部模板' : templates.data?.find((template) => String(template.id) === value)?.name ?? '全部模板'}</SelectValue></SelectTrigger><SelectContent><SelectItem value="all">全部模板</SelectItem>{templates.data?.map((template) => <SelectItem key={template.id} value={String(template.id)}>{template.name}</SelectItem>)}</SelectContent></Select>
      {hasPermission('group', 'read') && <Select value={groupId} onValueChange={(value) => { setGroupId(value ?? 'all'); resetPage() }}><SelectTrigger className="w-40"><SelectValue placeholder="全部组">{(value: string) => value === 'all' ? '全部组' : groups.data?.find((group) => String(group.id) === value)?.name ?? '全部组'}</SelectValue></SelectTrigger><SelectContent><SelectItem value="all">全部组</SelectItem>{groups.data?.map((group) => <SelectItem key={group.id} value={String(group.id)}>{group.name}</SelectItem>)}</SelectContent></Select>}
