    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        <div className="grid gap-4 py-2 md:grid-cols-2">
          ...
          <div className="space-y-2"><Label>任务模板 *</Label><Select value={templateVersionId ? String(templateVersionId) : ''} onValueChange={(value) => setTemplateVersionId(Number(value))}><SelectTrigger><SelectValue placeholder="选择已发布模板" /></SelectTrigger><SelectContent>{templates.data?.filter((item) => item.latestVersionId).map((item) => <SelectItem key={item.id} value={String(item.latestVersionId)}>{item.name}</SelectItem>)}</SelectContent></Select></div>
          <div className="space-y-2"><Label>执行人 *</Label><Select value={assigneeId ? String(assigneeId) : ''} onValueChange={(value) => setAssigneeId(Number(value))}><SelectTrigger><SelectValue placeholder="选择执行人" /></SelectTrigger><SelectContent>{users.data?.map((user) => <SelectItem key={user.id} value={String(user.id)}>{user.realName || user.username}</SelectItem>)}</SelectContent></Select></div>
