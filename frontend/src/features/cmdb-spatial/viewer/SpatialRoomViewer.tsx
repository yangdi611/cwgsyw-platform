...
export function SpatialRoomViewer({ roomId, canUpdate }: SpatialRoomViewerProps) {
  ...
  return <div className="-m-4 flex h-[calc(100vh-4rem)] flex-col md:-m-6">
    <header className="flex min-h-14 flex-wrap items-center gap-2 border-b border-v2-border bg-v2-surface px-4 py-2">
      ...
      <div className="order-3 flex w-full items-center gap-1 sm:order-none sm:w-auto"><Search className="ml-2 h-4 w-4 shrink-0 text-v2-muted" /><Input value={keyword} onChange={(event) => setKeyword(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') setSearchTerm(keyword.trim()) }} placeholder="搜索机柜或设备" className="h-8 w-full sm:w-44" />{searchTerm && <Button variant="ghost" size="icon" title="清除搜索" onClick={() => { setSearchTerm(''); setKeyword('') }}><X className="h-4 w-4" /></Button>}</div>
    ...
    {searchTerm && locations.length > 0 && <div className="absolute left-16 top-16 z-10 max-h-52 w-80 overflow-y-auto border border-v2-border bg-v2-surface shadow-v2-md"><div className="border-b border-v2-border px-3 py-2 text-xs text-v2-muted">搜索结果</div>{locations.map((item) => <button key={`${item.layoutId}-${item.elementId}-${item.targetCiInstanceId}`} className="block w-full px-3 py-2 text-left text-sm hover:bg-v2-surface-hover" onClick={() => openLocation(item)}><span className="block truncate text-v2-fg">{item.targetCiName}</span><span className="block truncate text-xs text-v2-muted">{item.pathLabel}</span></button>)}</div>}
